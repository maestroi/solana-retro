 ...
 * Seeds: ["catalog_root"]
 */
export function deriveCatalogRootPDA(programId: PublicKey = PROGRAM_ID): [PublicKey, number] {
 ...
 */
export function deriveCatalogPagePDA(
  pageIndex: number,
  ...
  return PublicKey.findProgramAddressSync(
    [CATALOG_PAGE_SEED, pageIndexBuffer],
    programId
  );
}
 ...
 */
export function deriveManifestPDA(
  cartridgeId: Uint8Array | Buffer,
  programId: PublicKey = PROGRAM_ID
): [PublicKey, number] {
  ...
  return PublicKey.findProgramAddressSync(
    [MANIFEST_SEED, Buffer.from(cartridgeId)],
    programId
  );
}
...
export function deriveChunkPDA(
  ...
  return PublicKey.findProgramAddressSync(
    [CHUNK_SEED, Buffer.from(cartridgeId), chunkIndexBuffer],
    programId
  );
}
 ...
 */
export function deriveAllChunkPDAs(
  cartridgeId: Uint8Array | Buffer,
  numChunks: number,
  programId: PublicKey = PROGRAM_ID
): Array<[PublicKey, number]> {
  const pdas: Array<[PublicKey, number]> = [];
