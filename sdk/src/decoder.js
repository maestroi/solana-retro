 ...
 */
export function decodeCatalogRoot(data) {
 ...
 */
export function decodeCatalogPage(data) {
    const buffer = Buffer.from(data);
    let offset = 8; // Skip discriminator
 ...
 */
export function decodeCartridgeManifest(data) {
    const buffer = Buffer.from(data);
    let offset = 8; // Skip discriminator
    const cartridgeId = new Uint8Array(buffer.subarray(offset, offset + 32));
    offset += 32;
 ...
 */
export function decodeCartridgeChunk(data) {
    const buffer = Buffer.from(data);
    let offset = 8; // Skip discriminator
    const cartridgeId = new Uint8Array(buffer.subarray(offset, offset + 32));
    offset += 32;
